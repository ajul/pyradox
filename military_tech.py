...
w = '{| class = "wikitable"\n'
w += '! rowspan = "2" | Tech level !! rowspan = "2" | Year !! rowspan = "2" | Morale !! rowspan = "2" | Combat<br/>Width !! rowspan = "2" | Military<br/>Tactics '
...
for level, tech in enumerate(tree.findAll('technology')):
    ...
    for key, value in tech.items():
        if key == 'year':
            ...
            stats[key] += value
    w += '|-\n'
    w += '| %(level)s || %(year)d || %(land_morale)0.2f || %(combat_width)d || %(military_tactics)0.2f ' % stats
