...
def process_idea_group(key, tree):
    ...
    ig_name = pyradox.yml.get_localization(key, localization_sources)

    if 'start' in tree:
        traditions = tree['start']
        title = pyradox.yml.get_localization('%s_start' % key, localization_sources) or key
        for bonus, value in traditions.items():
            add_bonus(bonus, title, value)

    ambitions = tree['bonus']
    title = pyradox.yml.get_localization('%s_bonus' % key, localization_sources) or key
    ...
    for idea, bonuses in tree.items():
        ...
        idea_name = pyradox.yml.get_localization(idea, localization_sources)
...
for bonus in sorted(bonus_sources.keys()):
    ...
    bonus_title = (
        pyradox.yml.get_localization('modifier_%s' % bonus, sources)
        or pyradox.yml.get_localization('yearly_%s' % bonus, sources)
        or pyradox.yml.get_localization(bonus, sources)
        )
    if not bonus_title:
